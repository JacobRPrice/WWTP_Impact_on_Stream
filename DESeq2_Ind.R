...
psf<-prune_samples(keepsamps,ps.ind)
psf
...
write.csv(as.data.frame(res.tab), file=file.path(output_path,"Ind_res.tab.DESeq.UPvsDOWN.txt"))
...
saveRDS(sigtab,file=file.path(output_path,"Ind_sigtab.RDS"))
write.csv(as.data.frame(sigtab), file=file.path(output_path,"Ind_sigtab.DESeq.UPvsDOWN.txt"))
...
# plot
plot.DESeq.UPvsDOWN.i<-
ggplot(sigtab, aes(x=Genus, y=log2FoldChange, color=Order)) + geom_point(size=6) +
	theme_bw() +
	theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5)) + 
	geom_hline(yintercept=0) +
	theme(axis.text.y = element_text(angle =90, hjust=0.5))
ggsave(file.path(figs_path,"Ind_DESeq.UPvsDOWN.eps"),plot.DESeq.UPvsDOWN.i,width=190,height=120,units="mm")
